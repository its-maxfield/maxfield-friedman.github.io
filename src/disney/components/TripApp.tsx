...
// Retained as a compact manual-data fallback while Queues owns primary navigation.
...
// Retained for history rendering while the unified Queues view owns LL actions.
// eslint-disable-next-line @typescript-eslint/no-unused-vars
...
function QueuesView({ parkId, now, online, onBook }: { parkId: ParkId; now: Date; online: boolean; onBook: (id?: string, attractionId?: string) => void }) {
  const { state, dispatch, refreshLive } = useDisneyStore();
  const day = state.days[parkId];
  const preferences = state.preferences[parkId];
  const [filter, setFilter] = useState("");
  const [mode, setMode] = useState<"all" | "standby" | "ll">("all");
  const [refreshing, setRefreshing] = useState(false);
  const [message, setMessage] = useState("");
  const held = day.reservations.filter((reservation) => reservation.status === "held").sort((a, b) => new Date(a.returnEnd).getTime() - new Date(b.returnEnd).getTime());
  const preference = (id: string) => preferences.find((item) => item.attractionId === id);
  const tierOrder: Record<PriorityTier, number> = { must: 0, nice: 1, convenient: 2, "dont-care": 3 };
  const visibleRides = attractionsForPark(parkId)
    .filter((ride) => ride.name.toLowerCase().includes(filter.toLowerCase()))
    .filter((ride) => mode === "all" || (mode === "ll" ? ride.lightningLane : day.attractionStates[ride.id]?.standbyMinutes !== undefined))
    .sort((a, b) => {
      const aPinned = day.pinnedAttractionIds.includes(a.id) ? 0 : 1;
      const bPinned = day.pinnedAttractionIds.includes(b.id) ? 0 : 1;
      const aHere = a.land === day.currentLand ? 0 : 1;
      const bHere = b.land === day.currentLand ? 0 : 1;
      const aPref = preference(a.id), bPref = preference(b.id);
      return aPinned - bPinned || aHere - bHere || tierOrder[aPref?.tier ?? "dont-care"] - tierOrder[bPref?.tier ?? "dont-care"] || (aPref?.rankWithinTier ?? 99) - (bPref?.rankWithinTier ?? 99) || a.name.localeCompare(b.name);
    });

  async function refresh() {
    setRefreshing(true); setMessage("");
    try {
      const count = await refreshLive(parkId);
      setMessage(`Updated ${count} attractions`);
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Live refresh failed");
    } finally { setRefreshing(false); }
  }

  function updateWait(ride: Attraction, standbyMinutes: number) {
    dispatch({ type: "UPDATE_STATUS", parkId, status: { attractionId: ride.id, standbyMinutes, lastUpdatedAt: now.toISOString(), source: state.simulation ? "mock" : "manual" }, label: `Corrected ${ride.name} wait → ${standbyMinutes}m` });
  }

  function updateLl(ride: Attraction, offsetMinutes: number) {
    const start = new Date(now.getTime() + offsetMinutes * 60000);
    dispatch({ type: "UPDATE_STATUS", parkId, status: { attractionId: ride.id, lightningLaneReturnStart: start.toISOString(), lightningLaneReturnEnd: new Date(start.getTime() + 3600000).toISOString(), lightningLaneAvailability: "available", lastUpdatedAt: now.toISOString(), source: state.simulation ? "mock" : "manual" }, label: `Corrected ${ride.name} LL return → ${formatTime(start)}` });
  }

  function addPlan(type: "EAT" | "SHOW") {
    const title = prompt(type === "EAT" ? "Meal or rest label" : "Show name");
    if (!title) return;
    const start = new Date(now.getTime() + 30 * 60000);
    dispatch({ type: "ADD_PLAN", parkId, plan: { id: uid(), type, title, start: start.toISOString(), end: new Date(start.getTime() + 60 * 60000).toISOString(), land: day.currentLand } });
  }

  function rideCard(ride: Attraction, compact = false) {
    const status = day.attractionStates[ride.id];
    const pref = preference(ride.id);
    const pinned = day.pinnedAttractionIds.includes(ride.id);
    const unavailable = status?.temporarilyUnavailable;
    return <section key={`${compact ? "pin" : "ride"}-${ride.id}`} className={`rounded-2xl border p-4 ${pinned ? "border-emerald-300/35 bg-emerald-300/5" : "border-white/10 bg-white/5"}`}>
      <div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="text-[10px] font-black uppercase tracking-wider text-white/40">{pref ? tierMeta[pref.tier].label : "Unrated"} · {ride.land}</p><h3 className="mt-1 font-black">{ride.name}</h3><p className={`mt-1 text-[11px] font-bold ${dataAgeMinutes(status?.lastUpdatedAt, now) >= 15 ? "text-rose-300" : "text-white/40"}`}>{status?.source === "live" ? "LIVE · " : status?.source === "manual" ? "MANUAL · " : ""}{freshnessLabel(status?.lastUpdatedAt, now)}</p></div><button aria-label={`${pinned ? "Unpin" : "Pin"} ${ride.name}`} onClick={() => dispatch({ type: "TOGGLE_PIN", parkId, attractionId: ride.id })} className={`grid size-11 shrink-0 place-items-center rounded-xl border ${pinned ? "border-emerald-300 bg-emerald-300 text-emerald-950" : "border-white/10 bg-white/5 text-white/55"}`}>{pinned ? "★" : "☆"}</button></div>
      <div className="mt-3 grid grid-cols-2 gap-2 text-sm"><div className="rounded-xl bg-black/15 p-3"><span className="block text-[10px] font-black uppercase text-white/40">Standby</span><strong className={unavailable ? "text-rose-300" : ""}>{unavailable ? status?.operatingStatus?.toUpperCase() ?? "UNAVAILABLE" : status?.standbyMinutes !== undefined ? `${status.standbyMinutes}m` : "Unknown"}</strong></div><div className="rounded-xl bg-black/15 p-3"><span className="block text-[10px] font-black uppercase text-white/40">LL return</span><strong>{!ride.lightningLane ? "Not offered" : status?.lightningLaneAvailability === "finished" ? "Finished" : status?.lightningLaneAvailability === "temp-full" ? "Temp full" : status?.lightningLaneReturnStart ? formatTime(status.lightningLaneReturnStart) : "Unknown"}</strong></div></div>
      {!compact && <><p className="mt-3 text-[10px] font-black uppercase tracking-wider text-white/40">Correct standby</p><div className="mt-2 flex gap-2 overflow-x-auto pb-1">{[5, 10, 15, 20, 30, 45, 60].map((wait) => <button key={wait} onClick={() => updateWait(ride, wait)} className="min-h-10 min-w-10 rounded-lg bg-white/10 text-xs font-black">{wait}</button>)}<button onClick={() => { const value = prompt("Standby minutes"); if (value && !Number.isNaN(Number(value))) updateWait(ride, Number(value)); }} className="min-h-10 min-w-10 rounded-lg bg-white/10 font-black">+</button></div>{ride.lightningLane && <><p className="mt-2 text-[10px] font-black uppercase tracking-wider text-white/40">Correct LL return</p><div className="mt-2 flex gap-2">{[[0, "NOW"], [15, "+15"], [30, "+30"], [60, "+60"]].map(([offset, label]) => <button key={label} onClick={() => updateLl(ride, Number(offset))} className="min-h-10 flex-1 rounded-lg bg-emerald-300/10 px-2 text-xs font-black text-emerald-200">{label}</button>)}</div></>}<div className="mt-3 grid grid-cols-2 gap-2"><button onClick={() => dispatch({ type: "SET_LOCATION", parkId, land: ride.land })} className={button}>I&apos;m here</button><button onClick={() => dispatch({ type: "UPDATE_STATUS", parkId, status: { attractionId: ride.id, temporarilyUnavailable: !status?.temporarilyUnavailable, operatingStatus: status?.temporarilyUnavailable ? "operating" : "down", lastUpdatedAt: now.toISOString(), source: state.simulation ? "mock" : "manual" } })} className={button}>{status?.temporarilyUnavailable ? "Mark operating" : "Mark down"}</button>{ride.lightningLane && !ride.singlePass ? <button onClick={() => onBook(undefined, ride.id)} className={button}>Record LL</button> : <span />}<button disabled={day.completedAttractionIds.includes(ride.id)} onClick={() => dispatch({ type: "COMPLETE_ATTRACTION", parkId, attractionId: ride.id, at: now.toISOString() })} className="min-h-11 rounded-xl bg-emerald-300 font-black text-emerald-950 disabled:opacity-35">Done</button></div></>}
    </section>;
  }

  return <div className="space-y-5">
    <header><div className="flex items-start justify-between gap-3"><div><h2 className="text-2xl font-black">Queues</h2><p className="mt-1 text-sm text-white/50">Live opportunities and your park-day shortlist.</p></div><button disabled={refreshing || !online || state.simulation} onClick={refresh} className="flex min-h-11 items-center gap-2 rounded-xl bg-emerald-300 px-3 text-sm font-black text-emerald-950 disabled:opacity-40"><RefreshCw size={16} className={refreshing ? "animate-spin" : ""} />{refreshing ? "Refreshing" : "Refresh live"}</button></div><p aria-live="polite" className="mt-2 text-xs text-white/45">{message || (day.lastLiveRefreshAt ? `Last pull ${freshnessLabel(day.lastLiveRefreshAt, now).toLowerCase()}` : online ? "Live queues have not been loaded yet." : "Offline · showing saved data")}</p></header>
    <section><div className="flex items-center justify-between"><h3 className="font-black">Held Lightning Lanes</h3><button onClick={() => onBook()} className="min-h-10 rounded-xl bg-emerald-300 px-3 text-xs font-black text-emerald-950">+ Booking</button></div><div className="mt-3 space-y-3">{held.length === 0 ? <p className="rounded-2xl border border-dashed border-white/15 p-4 text-sm text-white/45">No held Lightning Lanes.</p> : held.map((reservation) => <article key={reservation.id} className="rounded-2xl border border-sky-300/25 bg-sky-300/10 p-4"><p className="text-[10px] font-black uppercase tracking-wider text-sky-200">Return {formatTime(reservation.returnStart)}–{formatTime(reservation.returnEnd)}</p><h4 className="mt-1 font-black">{attractionById(reservation.attractionId)?.name}</h4><div className="mt-3 grid grid-cols-3 gap-2"><button onClick={() => onBook(reservation.id)} className={button}>Modify</button><button onClick={() => dispatch({ type: "SET_RESERVATION_STATUS", parkId, id: reservation.id, status: "redeemed", at: now.toISOString() })} className="min-h-11 rounded-xl bg-emerald-300 font-black text-emerald-950">Redeem</button><button onClick={() => dispatch({ type: "SET_RESERVATION_STATUS", parkId, id: reservation.id, status: "cancelled", at: now.toISOString() })} className={button}>Cancel</button></div></article>)}</div></section>
    <section className="rounded-2xl border border-white/10 bg-white/5 p-4"><h3 className="font-black">Day plans</h3><div className="mt-3 grid grid-cols-2 gap-2"><button onClick={() => addPlan("EAT")} className={button}>+ Meal / rest</button><button onClick={() => addPlan("SHOW")} className={button}>+ Show</button></div>{day.scheduledPlans.length > 0 && <div className="mt-3 space-y-2">{day.scheduledPlans.map((plan) => <div key={plan.id} className="flex items-center justify-between rounded-xl bg-black/15 p-3 text-sm"><div><strong>{plan.title}</strong><p className="text-xs text-white/45">{formatTime(plan.start)} · {plan.type}</p></div><button onClick={() => dispatch({ type: "REMOVE_PLAN", parkId, id: plan.id })} className="min-h-10 px-2 text-xs text-rose-200">Remove</button></div>)}</div>}<details className="mt-3 border-t border-white/10 pt-3"><summary className="cursor-pointer text-sm font-bold text-white/55">Action history ({day.history.length})</summary><div className="mt-2 space-y-2">{day.history.slice(-20).reverse().map((entry) => <p key={entry.id} className="text-xs text-white/45"><time className="mr-2 font-mono">{formatTime(entry.at)}</time>{entry.label}</p>)}</div></details></section>
    {day.pinnedAttractionIds.length > 0 && <section><h3 className="font-black">Pinned shortlist</h3><div className="mt-3 space-y-3">{day.pinnedAttractionIds.map((id) => attractionById(id)).filter((ride): ride is Attraction => Boolean(ride) && ride!.parkId === parkId).map((ride) => rideCard(ride, true))}</div></section>}
    <section><h3 className="font-black">Explore queues</h3><input value={filter} onChange={(event) => setFilter(event.target.value)} placeholder="Find an attraction" className={`${input} mt-3`} /><div className="mt-2 grid grid-cols-3 gap-2">{(["all", "standby", "ll"] as const).map((option) => <button key={option} onClick={() => setMode(option)} className={`${button} capitalize ${mode === option ? "border-emerald-300 bg-emerald-300/15 text-emerald-200" : ""}`}>{option === "ll" ? "LL" : option}</button>)}</div><div className="mt-3 space-y-3">{visibleRides.map((ride) => rideCard(ride))}</div></section>
  </div>;
}
...
function Dashboard() {
  ...
  return <main className="min-h-dvh bg-[#07110f] text-[#f7f2e7]"><RegisterPwa /><div className="mx-auto max-w-md px-4 pb-28 pt-[max(1.25rem,env(safe-area-inset-top))]"><header className="mb-5"><div className="flex items-center justify-between"><div><p className="text-[10px] font-black uppercase tracking-[0.2em] text-emerald-300">{state.simulation ? "Simulation mode" : day.config.date}</p><h1 className="mt-1 text-2xl font-black">Park Day Optimizer</h1></div><div className={`flex items-center gap-1 rounded-full border px-2.5 py-1.5 text-[10px] font-black ${online ? "border-emerald-700/50 bg-emerald-950 text-emerald-200" : "border-amber-300/30 bg-amber-300/10 text-amber-200"}`}>{online ? <Wifi size={12} /> : <CloudOff size={12} />}{online ? "ONLINE" : "OFFLINE"}</div></div><div className="mt-4 grid grid-cols-2 rounded-2xl bg-white/5 p-1">{(["disneyland", "california-adventure"] as ParkId[]).map((id) => <button key={id} onClick={() => { dispatch({ type: "SET_ACTIVE_PARK", parkId: id }); setView("now"); }} className={`min-h-11 rounded-xl px-2 text-xs font-black ${parkId === id ? "bg-emerald-300 text-emerald-950" : "text-white/45"}`}>{state.days[id].config.shortLabel}</button>)}</div></header>
    ...
    {view === "now" && <div className="space-y-4"><ActionCard action={recommendation} now={now} onComplete={(attractionId) => dispatch({ type: "COMPLETE_ATTRACTION", parkId, attractionId, at: now.toISOString() })} /><section className="rounded-3xl border border-emerald-700/50 bg-[#10221d] p-5"><div className="flex items-start justify-between"><div><p className="text-xs font-black uppercase tracking-[0.2em] text-emerald-300">Book next</p><h2 className="mt-2 text-xl font-black">{bookNext?.title ?? (day.nextLightningLaneEligibleAt && new Date(day.nextLightningLaneEligibleAt) > now ? "Wait for eligibility" : "No valuable selection yet")}</h2></div><Sparkles className="text-emerald-300" /></div><p className="mt-2 text-sm font-bold text-white/65">{bookNext?.subtitle ?? "Keep the official app authoritative."}</p><p className="mt-3 text-sm leading-5 text-white/55">{bookNext?.reason ?? "Refresh live queues or wait until the timer expires."}</p>{bookNext && <button onClick={() => setBooking({ open: true, attractionId: bookNext.attractionId })} className="mt-4 min-h-11 w-full rounded-xl bg-emerald-300 font-black text-emerald-950">Record booking</button>}</section><TimerCard parkId={parkId} now={now} /><div className="grid grid-cols-2 gap-3"><section className="rounded-2xl border border-white/10 bg-white/5 p-4"><MapPin size={18} className="text-sky-300" /><p className="mt-3 text-[10px] font-black uppercase tracking-wider text-white/45">Current land</p><p className="mt-1 font-black">{day.currentLand ?? "Set location"}</p></section><section className="rounded-2xl border border-white/10 bg-white/5 p-4"><BatteryMedium size={18} className="text-sky-300" /><p className="mt-3 text-[10px] font-black uppercase tracking-wider text-white/45">Energy</p><p className="mt-1 font-black capitalize">{day.fatigueLevel}</p></section></div><Progress parkId={parkId} /><section className="rounded-2xl border border-white/10 bg-white/5 p-4"><p className="text-xs text-white/45">Estimated effort to finish remaining Must Dos</p><p className="mt-1 text-xl font-black">~{Math.floor(remaining / 60)}h {remaining % 60}m</p></section></div>}
    {view === "queues" && <QueuesView parkId={parkId} now={now} online={online} onBook={(id, attractionId) => setBooking({ open: true, id, attractionId })} />}{view === "map" && <OfflineMapView parkId={parkId} now={now} />}{view === "settings" && <SettingsView parkId={parkId} onSetup={() => setSetupOverride(true)} />}
